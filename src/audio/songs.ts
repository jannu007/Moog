...
function makeClock(bpm: number): Clock {
  const secPerBeat = 60 / bpm;
  return (beat: number) => beat * secPerBeat;
}

function chord(notes: string[], startBeat: number, lenBeats: number, toSec: Clock, gapRatio = 1): NoteEvent[] {
  const dur = toSec(lenBeats) * gapRatio;
  return notes.map((n) => ({ note: nn(n), startSec: toSec(startBeat), durSec: dur }));
}

function note(n: string, startBeat: number, lenBeats: number, toSec: Clock, gapRatio = 0.92): NoteEvent {
  return { note: nn(n), startSec: toSec(startBeat), durSec: toSec(lenBeats) * gapRatio };
}

function arpeggio(notes: string[], startBeat: number, stepBeats: number, steps: number, toSec: Clock): NoteEvent[] {
...
function shift(events: NoteEvent[], byBeatsInSec: number): NoteEvent[] {
